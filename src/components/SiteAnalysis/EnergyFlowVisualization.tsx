...
const EnergyFlowVisualization: React.FC<EnergyFlowVisualizationProps> = ({ site }) => {
  ...
  const handleStorageClick = (e: React.MouseEvent, unitId: string) => {
    e.preventDefault();
    ...
    <div className="relative h-[300px] bg-background/50 rounded-lg p-6 border">
      <div className="absolute inset-0 flex items-center justify-between px-16">
              ...
              <Dialog key={unit.id}>
                <DialogTrigger asChild>
                  <Button
                    ...
                    onClick={(e) => handleStorageClick(e, unit.id)}
                  >
        ...
        <div className="absolute inset-0 pointer-events-none">
          {/* Solar to Storage Line */}
          <div className="absolute left-[15%] top-[30%] w-[35%] h-0.5 bg-gradient-to-r from-yellow-500 to-indigo-500 transform -rotate-6">
            <div className="absolute inset-0 bg-gradient-to-r from-yellow-500 to-indigo-500 animate-flow-right" />
          </div>
          
          {/* Wind to Storage Line */}
          <div className="absolute left-[15%] top-[60%] w-[35%] h-0.5 bg-gradient-to-r from-blue-500 to-indigo-500 transform rotate-6">
            <div className="absolute inset-0 bg-gradient-to-r from-blue-500 to-indigo-500 animate-flow-right" />
          </div>
          
          {/* Storage to Grid Line */}
          <div className="absolute right-[15%] top-1/2 w-[35%] h-0.5 bg-gradient-to-r from-indigo-500 to-red-500">
            <div className="absolute inset-0 bg-gradient-to-r from-indigo-500 to-red-500 animate-flow-right" />
          </div>
        </div>
      </div>
    </div>
